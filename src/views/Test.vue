 ...
 <div class="container">
  <header style="display: flex; flex-wrap: wrap; width: 100%; justify-content: space-between;">
   <div style="position: relative;">
    <h3 class="clickable" @click="subDropdown" style="display: inline-block" :class="{ active: subDropdownVis }">{{ subject.name }}</h3>
    <ul v-if="subDropdownVis" class="dropdown">
     <li class="clickable" v-for="(subj, idx) in testState.testData.subjects" :key="idx" @click="selectSubject(idx)">{{ subj.name }}</li>
    </ul>
    |
    <h3 class="clickable" @click="secDropdown" style="display: inline-block" :class="{ active: secDropdownVis }">{{ section.name }}</h3>
    <ul v-if="secDropdownVis" class="dropdown">
     <li class="clickable" v-for="(sec, idx) in subject.sections" :key="idx" @click="selectSection(idx)">{{ sec.name }}</li>
    </ul>
   </div>
   <h3>{{ timeLeft }}</h3>
  </header>
...
<script setup>
 ...
 function clearQ()
 {
  testState.answers[testState.currentSubjectIndex][testState.currentSectionIndex][testState.currentQuestionIndex] = null
 }
